...
def sql_insert(table_name: str, element: dict, unique_columns: List[str] = None) -> str:
    ...
    if table_name == "answers_1293732":
        ...
        if "background_TRG_hotline" in element:
            updating = f"ON CONSTRAINT unique_{table_name} DO UPDATE SET background_trg_hotline = ARRAY[{element['background_TRG_hotline']}];"
            print(updating)
        else:
            updating = "DO NOTHING"

        conflict = f" ON CONFLICT {updating};"
